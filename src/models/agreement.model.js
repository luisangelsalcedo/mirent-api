 ...
 * ACTIVO: Se puede modificar solo el estado, la firma, generar pagos y notificaciones
...
agreementSchema.pre("updateOne", async function (next) {
  ...
  const { disabled, active, archived } = agreement.status;
  ...
  // ACTIVO
  if (active) {
    const { status, sign, rents, notification, ...res } = this.body;
